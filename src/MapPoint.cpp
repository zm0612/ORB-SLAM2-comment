    ...
    mpReplaced(static_cast<MapPoint*>(NULL)), mfMinDistance(0), mfMaxDistance(0), mpMap(pMap)
{
    ...
    unique_lock<mutex> lock(mpMap->mMutexPointCreation);//当运行到这个代码时候，对括号中的变量进行加锁，如果这个代码没有
                                                        //执行完毕，别的调用这个变量的线程就得等待
    mnId=nNextId++;
}
...
{
    Pos.copyTo(mWorldPos);
    cv::Mat Ow = pFrame->GetCameraCenter();//当前帧相机中心到世界参考系的平移矩阵
    mNormalVector = mWorldPos - Ow;// 世界坐标系下相机到3D点的向量
    mNormalVector = mNormalVector/cv::norm(mNormalVector);// 世界坐标系下相机到3D点的单位向量,作为观测方向

    cv::Mat PC = Pos - Ow;
    const float dist = cv::norm(PC);//求MapPoint到相机距离的范数
    const int level = pFrame->mvKeysUn[idxF].octave;//去畸变之后的关键点在金字塔的哪一层
    const float levelScaleFactor =  pFrame->mvScaleFactors[level];//当前level层相对于原始图像的缩放比例
...
{
    unique_lock<mutex> lock(mMutexFeatures);
    if(mObservations.count(pKF)) //mObservations.count(pKF)是查找mObservations中是否有pKF
        return;//说明这个MapPoint已经在这个地图的观测中
