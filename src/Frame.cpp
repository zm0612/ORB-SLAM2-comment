 ...
 */
 //用Frame类型去构造Frame类型，将当前的Frame中的参数，全部复制给前一个Frame
...
{
    //将当前Frame的mGrid复制给上一个Frame中的mGrid
    for(int i=0;i<FRAME_GRID_COLS;i++)
        for(int j=0; j<FRAME_GRID_ROWS; j++)
            mGrid[i][j]=frame.mGrid[i][j];

    if(!frame.mTcw.empty())
        SetPose(frame.mTcw);//设置当前帧的姿态
}
...
{
    // Frame ID
    mnId=nNextId++;//将nNextId赋值给mnId，然后nNextId等于下一帧的Id编号

    // Scale Level Info
    mnScaleLevels = mpORBextractorLeft->GetLevels();//获取金字塔的层数
    mfScaleFactor = mpORBextractorLeft->GetScaleFactor();//获得每层之间的缩放比例
    mfLogScaleFactor = log(mfScaleFactor);
    mvScaleFactors = mpORBextractorLeft->GetScaleFactors();//每层相对于原始图像的缩放比例
    mvInvScaleFactors = mpORBextractorLeft->GetInverseScaleFactors();////每层的相对于原始图像的缩放比例的倒数
...
{
    // Frame ID
    mnId=nNextId++;//静态成员变量，存储的是Frame的Id索引号

    // Scale Level Info
    mnScaleLevels = mpORBextractorLeft->GetLevels();//获取金字塔的层数
    mfScaleFactor = mpORBextractorLeft->GetScaleFactor();//获取每层金字塔之间的缩放比例
    mfLogScaleFactor = log(mfScaleFactor);//对缩放比例取log函数
    mvScaleFactors = mpORBextractorLeft->GetScaleFactors();//获取每一层相对于原始图像的缩放比例
    mvInvScaleFactors = mpORBextractorLeft->GetInverseScaleFactors();//获取每层的相对于原始图像的缩放比例的倒数
    mvLevelSigma2 = mpORBextractorLeft->GetScaleSigmaSquares();//获取图像的缩放比例sigma的平方
    mvInvLevelSigma2 = mpORBextractorLeft->GetInverseScaleSigmaSquares();//获取图像的缩放比例sigma的平方的倒数
    ...
    mvuRight = vector<float>(N,-1);//由于是单目的所以没有右边相机信息，也没有深度信息
    mvDepth = vector<float>(N,-1);

    mvpMapPoints = vector<MapPoint*>(N,static_cast<MapPoint*>(NULL));//将mvpMapPoints初始化为存放指向每一个MapPoint的指针的容器
    mvbOutlier = vector<bool>(N,false);//初始化异常值的容器
 ...
 */
 //根据成员变量mTcw计算出mRcw、mtcw和mRwc、mOw
void Frame::UpdatePoseMatrices()
{
    // [x_camera 1] = [R|t]*[x_world 1]，坐标为齐次形式
    // x_camera = R*x_world + t
    mRcw = mTcw.rowRange(0,3).colRange(0,3);//提取0到3行中0到3列的所有元素
    mRwc = mRcw.t();
...
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit)
{
    ...
    const float u=fx*PcX*invz+cx;//归一化的坐标值
...
//这个函数的主要作用是，将关键点对应的到Grid的中的位置（posX, posY）计算出来
bool Frame::PosInGrid(const cv::KeyPoint &kp, int &posX, int &posY)
{
    //确定关键点在哪一个Grid中
    posX = round((kp.pt.x-mnMinX)*mfGridElementWidthInv);//round四舍五入到最近邻的整数
...
void Frame::ComputeBoW()
{
    if(mBowVec.empty())
    {
        ...
        mpORBvocabulary->transform(vCurrentDesc,mBowVec,mFeatVec,4);//Transform a set of descriptors into a bow vector and a feature vector
    }
}
